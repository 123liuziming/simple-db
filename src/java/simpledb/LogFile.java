...
public class LogFile {
    ...
    private void doRollback(long tid) throws IOException {
        // 读日志文件，找到所有和tid相关的更新记录
        long firstLogRecordPos = tidToFirstLogRecord.get(tid);
        // 从后往前读，把所有的update record都rollback
        raf.seek(raf.length() - LONG_SIZE);
        long logPos = raf.readLong();
        while (firstLogRecordPos < logPos) {
            raf.seek(logPos);
                ...
                while (pos >= lastCheckPoint) {
                    ...
                    switch (raf.readInt()) {
                        ...
                        case ABORT_RECORD:
                            break;
                        case COMMIT_RECORD:
                            committedTransactions.add(raf.readLong());
                            break;
                        case BEGIN_RECORD:
                            aliveTransactions.add(raf.readLong());
                            break;
                        case CHECKPOINT_RECORD:
                            raf.seek(raf.getFilePointer() + LONG_SIZE);
                            int activeTransactionsNum = raf.readInt();
                            for (int i = 0; i < activeTransactionsNum; ++i) {
                                long tid = raf.readLong();
                                long firstPos = raf.readLong();
                                aliveTransactions.add(tid);
                                tidToFirstLogRecord.put(tid, firstPos);
                            }
                            break;
                        default:
                            throw new IOException("Recover Error");
                    }
                    if (pos > LONG_SIZE) {
                    ...
                    switch (raf.readInt()) {
                        ...
                        case ABORT_RECORD: {
                            ...
                            if (!tidToFirstLogRecord.containsKey(tid)) {
                                throw new IOException("ABORT: transaction " + tid + "is not live");
                            }
                            pos = raf.getFilePointer();
                            doRollback(tid);
                            aliveTransactions.remove(tid);
                            break;
                        }
                        case COMMIT_RECORD: {
                            long tid = raf.readLong();
                        ...
                        case CHECKPOINT_RECORD: {
                            raf.seek(raf.getFilePointer() + LONG_SIZE);
                            int activeTransactionsNum = raf.readInt();
                            pos = raf.getFilePointer() + activeTransactionsNum * LONG_SIZE * 2;
                            break;
                        }
                        default:
                            throw new IOException("Redo Error");
                    }
                    pos += LONG_SIZE;
                }
                // 求没提交的所有事务
                aliveTransactions.removeAll(committedTransactions);
                for (long tid : aliveTransactions) {
                    if (tidToFirstLogRecord.containsKey(tid)) {
                        doRollback(tid);
                    }
                }
